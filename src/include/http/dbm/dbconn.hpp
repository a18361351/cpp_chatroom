...
struct DBConn {
   private:
    ...
    void ReconnectImpl(boost::mysql::error_code &err, std::string_view username, std::string_view password,
                       std::string_view db_name);

   public:
    DBConn(boost::asio::io_context &ctx, boost::asio::ssl::context &ssl_ctx, std::string_view mysql_addr,
           uint mysql_port)
        : conn_(ctx, ssl_ctx), mysql_addr_(mysql_addr), mysql_port_(mysql_port) {}
    bool valid_{false};
    boost::mysql::tcp_ssl_connection conn_;
    boost::mysql::statement login_check_stmt_;
    boost::mysql::statement register_stmt_;
    boost::mysql::statement exist_check_stmt_;
    std::string_view mysql_addr_;
    uint mysql_port_;

    // 连接与关闭的函数
    void Connect(boost::mysql::error_code &err, std::string_view username, std::string_view password,
                 std::string_view db_name) {
