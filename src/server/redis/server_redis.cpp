...
namespace chatroom::backend {
    ...
    std::string RedisMgr::SendToMsgQueue(std::string_view server_id, uint64_t from, uint64_t to, std::string_view content, int queue_max_len) {
    ...
    bool RedisMgr::UpdateUserStatus(std::string_view server_id, uint64_t uid) {
            std::string key_name = "status:"; key_name += std::to_string(uid);
            thread_local std::unordered_map<string, string> user_data = {{"server_id", "unset"}, {"status", "online"}};
            user_data["server_id"] = server_id;
            auto ans = GetRedis().hsetex(key_name, user_data.begin(), user_data.end(), std::chrono::seconds(30), sw::redis::HSetExOption::ALWAYS);
            return ans == 1;
        }
