...
bool GatewayHelper::Login(const string &username, const string &passcode, string &token, string &server_addr,
                          uint64_t &uid) {
...
void AsyncClient::ConnectedHandler() {
    ...
    running_ = true;
    worker_ = std::thread([self = shared_from_this()]() { self->WorkerFn(); });
}
...
void AsyncClient::ReceiveHead() {
    ...
    // 消息头部
    // sock_.async_receive(boost::asio::buffer(recv_buf_.data_ + recv_buf_.cur_pos_, HEAD_LEN - recv_buf_.cur_pos_),
    // cb);
    boost::asio::async_read(sock_,
                            boost::asio::buffer(recv_buf_.data_ + recv_buf_.cur_pos_, HEAD_LEN - recv_buf_.cur_pos_),
...
void AsyncClient::ReceiveContent() {
    ...
    // 消息体
    // sock_.async_receive(boost::asio::buffer(recv_buf_.data_ + recv_buf_.cur_pos_, recv_buf_.ctx_len_ + HEAD_LEN -
    // recv_buf_.cur_pos_), cb);
    boost::asio::async_read(
        sock_,
        boost::asio::buffer(recv_buf_.data_ + recv_buf_.cur_pos_, recv_buf_.ctx_len_ + HEAD_LEN - recv_buf_.cur_pos_),
        boost::asio::bind_executor(strand_, cb));
}
...
void AsyncClient::WorkerJoin() { worker_.join(); }
bool AsyncClient::Running() const { return running_; }
}  // namespace chatroom::client